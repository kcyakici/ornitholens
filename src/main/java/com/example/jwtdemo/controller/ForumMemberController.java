...
public class ForumMemberController {
    ...
    @Autowired
    public ForumMemberController(UserDetailsServiceImpl userDetailsService) {
...
//    public ResponseEntity<String> updateScore(@AuthenticationPrincipal User requestingUser, ScoreUpdateRequest scoreUpdateRequest) {
//        ForumMember forumMember = userDetailsService.findForumMemberByUsername(requestingUser.getUsername());
//        int newScore = scoreUpdateRequest.newScore();
//        System.out.println("New score to be set: " + newScore);
//        userDetailsService.updateScore(forumMember.getId(), newScore);
